...
# 适配adasub
def scale_matrix(M, epsilon=1e-6):
    """
    Implements the Scale(.) function: M / (||M||_F + epsilon)
    """
    norm = torch.norm(M, p='fro')
    return M / (norm + epsilon)
...
class OurTrainer(Trainer):
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                self.writer.add_scalar('train_loss', tr_loss_step.item(), total_steps)
    ...
    # ============================================================
    # AdaSub-ZO Implementation: Phase 1 (Subspace Learning)
    # ============================================================
    # def train_subspace_basis(self, model, inputs):
    #     """
    #     Phase 1: Learn the subspace basis U and V.
    #     Objective: min_{U, V} Loss(theta + alpha * Scale(U @ V.T))
    #     """
    #     args = self.args
    ...
    #     # 2. Optimization Loop for U and V
    #     # We use a simple ZO-SGD to update U and V
    #     model.eval() # Ensure dropout is off for consistent loss measurement
        
    #     # Determine which params participate
    #     target_params = []
    #     for name, param in model.named_parameters():
    #         if param.requires_grad and len(param.shape) == 2:
    ...
    #         # --- a. Sample Perturbations for U and V ---
    #         # We need to perturb ALL U's and V's simultaneously to get one loss estimate
    #         # Z_U, Z_V ~ N(0, I)
    #         perturbations = {}
    #         for name in target_params:
    #             U = self.p_state[name]['U']
    #             V = self.p_state[name]['V']
    #             perturbations[name] = {
    #                 'Z_U': torch.randn_like(U),
    #                 'Z_V': torch.randn_like(V)
    #             }

    #         # --- b. Apply Perturbation to Model Weights (Forward +) ---
    #         # theta' = theta + alpha * Scale((U + sigma*Z_U)(V + sigma*Z_V)^T)
            
    #         # Save original weights to revert later
    #         original_weights = {} 
    ...
    #                 # Construct the low-rank update matrix
    #                 update_matrix = U_new @ V_new
                    
    #                 # Apply Scale() and Step Size (Alpha)
    #                 # Note: We add this to the ANCHOR (theta_t), which is currently param.data
    #                 param.data = param.data + args.adasub_alpha * scale_matrix(update_matrix)

    #         # --- c. Compute Loss (Forward Pass) ---
    ...
    #         # --- e. Estimate Gradients for U and V ---
    #         # Ideally we need two-sided, but for speed let's assume loss_orig approx const or use one-sided diff
    #         # To be more accurate like MeZO:
    #         # loss_grad ~ (Loss_pos - Loss_current) / sigma
    #         # Let's get Loss_current (optional, usually we can skip if assuming descent)
    #         # For simplicity and speed, let's assume we want to minimize Loss_pos directly via random search direction.
    #         # Gradient Estimator: g = (Loss_pos - Loss_prev) / sigma * Z
    #         # But here we simply update in the direction that minimizes loss.
    #         # Simplified ZO-Update rule: U <- U - lr * (Loss_pos - Baseline) * Z_U / sigma
            
    #         # To make it robust, let's do antithetic sampling (Two-sided) for better gradient approximation
    #         # ... Re-apply Negative Perturbation ...
    #         for name, param in model.named_parameters():
    #             if name in target_params:
    #                 U_new = self.p_state[name]['U'] - args.adasub_sigma * perturbations[name]['Z_U']
    #                 V_new = self.p_state[name]['V'] - args.adasub_sigma * perturbations[name]['Z_V']
    #                 update_matrix = U_new @ V_new
    #                 param.data = original_weights[name] + args.adasub_alpha * scale_matrix(update_matrix)
    ...
    #         # --- f. Update U and V ---
    #         # projected_grad = (loss_pos - loss_neg) / (2 * sigma)
    #         scalar_grad = (loss_pos - loss_neg) / (2 * args.adasub_sigma)
            
    #         for name in target_params:
    #             # Gradient Descent on U and V
    #             # U = U - lr * scalar_grad * Z_U
    #             # V = V - lr * scalar_grad * Z_V
    #             self.p_state[name]['U'] = self.p_state[name]['U'] - args.adasub_lr * scalar_grad * perturbations[name]['Z_U']
    #             self.p_state[name]['V'] = self.p_state[name]['V'] - args.adasub_lr * scalar_grad * perturbations[name]['Z_V']

        # 3. (Optional) Orthogonalize / QR Decomposition at the end
        # This aligns with SubZero's requirement for U and V to be bases
        # Although your idea is to "learn" them, keeping them well-conditioned is good.
        # for name in target_params:
        #     with torch.no_grad():
    ...
    # 适配adasub
    @torch.no_grad()
    def zo_adasubspace_step(self, model, inputs):
        """
        Estimate gradient by subzero (Modified for AdaSub-ZO). 
        """
        args = self.args
        ...
        if not self.p_state: 
             for name, param in model.named_parameters():
                if param.requires_grad and len(param.shape) == 2:
                     # Initial dummy init, will be overwritten by train_subspace_basis immediately
        ...
        for name, param in model.named_parameters():
            if param.requires_grad:
                ...
                if len(torch.squeeze(param.data).shape) == 2:
                    # Retrieve the LEARNED U and V
                    U = self.p_state[name]['U']
                    V = self.p_state[name]['V']
                    
                    self.named_parameters_to_optim.append((name, param, U, V))
                else:
                    # Fallback for vectors (bias/layernorm) - just use identity/standard ZO
                    self.named_parameters_to_optim.append((name, param, torch.Tensor([1.]), torch.Tensor([1.])))
