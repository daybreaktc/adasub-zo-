...
class OurArguments(TrainingArguments):
    ...
    grad_clip_value: float = 0.0   # Clip ZO gradient estimates (0 = disabled). Typical: ~500 for eps=1e-3
    # ------------------------------------
...
def main():
    ...
    # 1. 先生成基础 tag
    base_tag = f"{args.trainer}-{args.task_name}-{args.template_ver}-{args.model_name.split('/')[-1]}-OPTIM_{args.mode}-STEP{args.max_steps}-{args.optimizer}-momen{args.momentum}-LR{args.learning_rate}-{args.lr_scheduler_type}-ZOEPS{args.zo_eps}-T{args.update_interval}-gauss_rank{args.gauss_rank}-Q{args.q}-bs{args.per_device_train_batch_size}-gradAccumulation{args.gradient_accumulation_steps}"
    # 2. 如果命令行传入了 --tag，则将其拼接到后面；否则直接使用基础 tag
    # 注意：这里我们假设您通过命令行传入的 tag 存储在 args.tag 中，但在被覆盖前我们需要先判断它是否存在
    # 由于 HfArgumentParser 的机制，我们需要稍微变通一下，或者直接利用 args.tag (如果此时尚未被覆盖)
    # 更稳妥的写法（因为 args.tag 在上面已经被定义为参数了）：
    if args.tag:
        args.tag = base_tag + "-" + args.tag
    else:
        args.tag = base_tag
